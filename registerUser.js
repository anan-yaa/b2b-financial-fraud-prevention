...
async function main() {
  try {
    ...
    const ca = new FabricCAServices(
      caInfo.url,
      { trustedRoots: caInfo.caName, verify: false },
      caInfo.caName,
    );
    ...
    if (!adminIdentity) {
      console.log(
        '❌ An identity for the admin user "Admin@org1.example.com" does not exist in the wallet',
      );
    ...
    console.log(
      "💾 Successfully enrolled and imported user:",
      userId,
      "into the wallet",
    );
    ...
    for (const role of roles) {
      ...
      console.log(
        `💾 Successfully enrolled and imported ${role} user:`,
        roleId,
        "into the wallet",
      );
    }
