...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 409:
          if (data.message && data.message.includes("Double Disbursement")) {
            toast.error(
              "Double Disbursement Blocked: This invoice has already been processed",
            );
          } else if (data.message && data.message.includes("Discovery Error")) {
            toast.error(
              "Discovery Error: There was an issue with the blockchain transaction",
            );
          } else {
      ...
      toast.error(
        "Network Error - Unable to connect to the server. Check if backend is running on port 3000.",
      );
    } else {
...
export const vendorAPI = {
  ...
  updateVendor: (vendorId, vendorData) =>
    api.put(`/vendor/${vendorId}`, vendorData),
...
export const buyerAPI = {
  // Register new buyer
  registerBuyer: (buyerData) => api.post("/buyer", buyerData),
};
